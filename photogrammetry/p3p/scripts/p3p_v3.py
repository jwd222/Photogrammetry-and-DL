...
class GLWidget(QGLWidget):
    ...
    def paintGL(self):
        ...
        if self.show_points:
            ...
            for point in self.world_points:
                ...
                gluSphere(self.quadric, 0.1, 16, 16) # Replaced glutSolidSphere
                glPopMatrix()
        ...
        if self.camera_found and self.show_camera:
            ...
            # Draw camera body (a sphere)
            glColor3f(0.2, 0.4, 0.8) # Blueish
            gluSphere(self.quadric, 0.3, 16, 16) # Replaced glutSolidSphere
            
            # Draw camera lens/frustum (a cone)
            glPushMatrix()
            glTranslatef(0, 0, 0.3) # Move slightly forward from sphere center
            glRotatef(90, 1, 0, 0) # Rotate to make cone point along Z-axis (initially points along Y for gluCylinder)
            gluCylinder(self.quadric, 0.4, 0.0, 0.8, 16, 16) # Replaced glutWireCone
            gluDisk(self.quadric, 0, 0.4, 16, 1) # Base of the cone
            glPopMatrix()
        ...
        if self.show_reprojection and self.camera_found:
            ...
            for i in range(len(self.world_points)):
                ...
                gluSphere(self.quadric, 0.05, 10, 10) # Replaced glutSolidSphere
                glPopMatrix()
            glEnable(GL_LIGHTING)
    ...
    def wheelEvent(self, event):
        ...
        self.zoom = max(0.1, min(self.zoom - delta, 100.0)) # Adjusted zoom range
        self.update()
    ...
    def compute_p3p(self):
        ...
        # These are example values. In a real scenario, they come from camera calibration.
        ...
        # Ensure world_points and image_points are in the correct shape and type
        # solvePnPGeneric expects world points as (N, 1, 3) and image points as (N, 1, 2)
        # and both as float32 or float64.
    ...
    def set_point_count(self, n):
        ...
        # These are relative to the origin, not large EPSG:28992 values
        ...
        if n > old_n:
            ...
            for i in range(old_n, n):
                # Simple default values for new points
                self.world_points = np.append(self.world_points, [new_3d_point + i * 0.1], axis=0)
                self.image_points = np.append(self.image_points, [new_2d_point + i * 10], axis=0)
        else:
            # Truncate
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.world_group = QGroupBox("World Points (3D Coordinates)") # Removed EPSG:28992 label for clarity
        ...
        # Add compute button
        compute_btn = QPushButton("Compute Camera Pose (PnP)") # Changed label to PnP
        ...
        points_cb = QPushButton("Toggle World Points") # Changed label
        points_cb.clicked.connect(lambda: self.toggle_option('points'))
        vis_layout.addWidget(points_cb)
        
        reproj_cb = QPushButton("Toggle Reprojection Markers") # Changed label
